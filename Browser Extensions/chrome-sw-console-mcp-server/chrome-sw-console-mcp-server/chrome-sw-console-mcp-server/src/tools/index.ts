...
import {
  formatConsoleMessagesMarkdown,
  formatConsoleMessagesJSON,
  formatTargetsMarkdown,
  formatStatusMarkdown,
} from "../services/formatters.js";
import {
  ListTargetsInputSchema,
  ConnectToTargetInputSchema,
  StartMonitoringInputSchema,
  StopMonitoringInputSchema,
  GetConsoleLogsInputSchema,
  ClearLogsInputSchema,
  GetStatusInputSchema,
  DisconnectInputSchema,
  type ListTargetsInput,
  type ConnectToTargetInput,
  type GetConsoleLogsInput,
  type GetStatusInput,
...
Error Handling:
  ...
  - Returns error if connection to Chrome DevTools fails`,
    ...
    async (input: { response_format?: "markdown" | "json"; chrome_host?: string; chrome_port?: number }) => {
      const params: ListTargetsInput = {
...
Error Handling:
  ...
  - Returns error if Chrome connection fails`,
    ...
    async (input: { target_id: string; chrome_host?: string; chrome_port?: number }) => {
      const params: ConnectToTargetInput = {
...
Error Handling:
  ...
  - Returns error if not monitoring (start with chrome_sw_start_monitoring)`,
    ...
    async (input: { 
      ...
      const params: GetConsoleLogsInput = {
        response_format: input.response_format ?? "markdown",
...
Error Handling:
  - Always succeeds, returns current status`,
    ...
    async (input: { response_format?: "markdown" | "json" }) => {
      const params: GetStatusInput = {
        response_format: input.response_format ?? "markdown",
      };
      try {
        const client = getChromeClient();
