 ...
 */
export function formatConsoleMessagesMarkdown(logs, limit) {
    if (!logs.length) {
 ...
 */
export function formatConsoleMessagesJSON(logs, limit) {
    const displayLogs = limit ? logs.slice(0, limit) : logs;
    return {
        total: logs.length,
 ...
 */
export function formatTargetsMarkdown(targets) {
    if (!targets.length) {
    ...
    const serviceWorkers = targets.filter((t) => t.type === "service_worker" || t.description?.includes("service worker"));
    const extensions = targets.filter((t) => t.type === "page" && t.url.startsWith("chrome-extension://"));
    const others = targets.filter((t) => !serviceWorkers.includes(t) && !extensions.includes(t));
 ...
 */
function formatTargetEntry(target) {
